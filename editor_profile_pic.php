...
if(isset($_POST['up'])) {
   if(!empty($name)){
       if ($size < $max_size && ($extension == 'jpg' || $extension == 'jpeg' || $extension == 'png') && ($type == 'image/jpeg' || $type == 'image/png')) {
           $target = "uploads/" . editor_get_field("F_name") . "_" . basename($name);
           $con = mysqli_connect("localhost", "root", "", "hostel");


           $s = "UPDATE `editor` SET `profile_pic` = '" . editor_get_field("F_name") . "_$name' WHERE `ESSN` = '" . editor_get_field('ESSN') . "'  ";
...
<div class="row">
    <div class="col-lg-12"><div>
            <a href="editor_profile.php"><button class="btn btn-large btn-primary" style="float: left;margin-left: 100px;margin-top:20px"><i class="fa fa-arrow-left"></i> Profile</button></a>
            <a href="editor_logout.php"><button class="btn btn-large btn-danger" style="float: right;margin-right: 100px;margin-top:20px"><i class="fa fa-power-off"></i>  Log Out</button></a>
        </div></div>
</div>
